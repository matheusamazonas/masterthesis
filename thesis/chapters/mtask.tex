...
The \gls{mTask} library provides functions to interact with devices. Their type signatures can be seen in Listing \ref{mtask_devices}. Some of these functions take an \texttt{MTaskDevice} as input (also seen in Listing \ref{mtask_devices}). This \acs{adt} holds an \gls{mTask} device information, including its data and channels. 
